...
class App:
    ...
    def registration_form(self):
        ...
        repeat_password_label = Label(self.__window, text="Имя пользователя", font=("Arial", 16))
        ...
        regestration_complete = Button(self.__window, text="Регистрация",width=15, font=("Arial", 16))
        regestration_complete.place(anchor="center", relx=0.5, rely=0.90)
    ...
    def valid_info(self, name, phone, password, repeat_password):
        if len(name) == 0 or len(phone) == 0 or len(password) == 0 or len(repeat_password)== 0:
            ...
            message_info= "Одно или несколько полей не заполнены!"
